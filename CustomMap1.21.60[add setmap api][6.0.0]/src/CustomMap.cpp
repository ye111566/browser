...
void RemoteCallExport() {
	...
	auto setMap = [](const std::string& filepath, int mapid, bool alpha) {
	    ...
	    // 获取 MapItemSavedData 的指针（getMapSavedData 返回的是指针）
	    auto* mapd = level._getMapDataManager().getMapSavedData(uid);
	    if (!mapd) {  // 检查指针是否有效
	        return -1LL;
	    }
	
	    // setScale 和 MapSetPixels 需要引用，所以解引用指针
	    setScale(*mapd, 4);  // *mapd 变成 MapItemSavedData&
	    custom_map::MapSetPixels(*mapd, ifs, alpha);  // *mapd 变成 MapItemSavedData&
	
	    mapd->save(level.getLevelStorage());
	
	    // ClientboundMapItemDataPacket 需要 gsl::not_null<MapItemSavedData*>
	    ClientboundMapItemDataPacket(gsl::not_null(mapd), level).sendToClients();
